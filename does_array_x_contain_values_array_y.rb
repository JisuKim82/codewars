...
# This method will always assume that an array is passed in and will return true if all values within the passed in array are present within the current array.
...
class Array
  def contains_all?(other_array)
    other_array.each do |x|
      return false unless self.include?(x)
    end
    true
  end
end